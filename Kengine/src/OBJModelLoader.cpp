...
DirectX::XMFLOAT3 OBJModelLoader::ParseFloat3(std::string verts)
{
...
DirectX::XMFLOAT2 OBJModelLoader::ParseFloat2(std::string verts)
{
...
OBJModelLoader::FaceFormat OBJModelLoader::DetectFaceFormat(std::string face)
{
...
void OBJModelLoader::ParseFace(std::string face)
{
	...
	switch (format)
	{
	case OBJModelLoader::FaceFormat::UNKNOWN:
		break;
	case OBJModelLoader::FaceFormat::V:
		break;
	case OBJModelLoader::FaceFormat::V_VT:
		replaced = sscanf_s(face.c_str(), "%u/%u %u/%u %u/%u",
			&p1.v, &p1.vt, &p2.v, &p2.vt, &p3.v, &p3.vt);
		if (replaced != 6)
			return;
		break;
	case OBJModelLoader::FaceFormat::V_VN:
		replaced = sscanf_s(face.c_str(), "%u//%u %u//%u %u//%u",
			&p1.v, &p1.vn, &p2.v, &p2.vn, &p3.v, &p3.vn);
		if (replaced != 6)
			return;
		break;
	case OBJModelLoader::FaceFormat::V_VT_VN:
	...
	case OBJModelLoader::FaceFormat::FORMAT_ERROR:
	default:
		return;
	}
...
std::vector<std::string> OBJModelLoader::Split(std::string s, std::string delimiter)
{
...
OBJModelLoader::OBJModelLoader(std::string path)
{
	LoadModelData(path);
}

void OBJModelLoader::LoadModelData(std::string path)
{
	using namespace std;
